...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadData = () => {
    ...
    // Load Pluely API enabled state (always false)
  ...
  // Sync STT mode to Rust backend when provider changes
  useEffect(() => {
    if (selectedSttProvider.provider === "in-process") {
      invoke("set_stt_mode", { useLocal: true }).catch(console.error);
    } else if (selectedSttProvider.provider) {
      invoke("set_stt_mode", { useLocal: false }).catch(console.error);
    }
